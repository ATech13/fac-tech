...
import { createEmptyInvoice, getInoiceByEmail } from "./action";
...
export default function Home() {
  ...
  useEffect(() => {
    const fetchInvoices = async () => {
      try {
        const data = await getInoiceByEmail(email)
        if (data) {
          setInvoices(data)
        }
      } catch (error) {
      ...
      <div className="flex flex-col space-y-4">
        ...
        <dialog id="my_modal_3" className="modal">
          <div className="modal-box">
            ...
            {!isNameValid && <p className="mb-4 text-sm">Le nom de la facture est requis et doit être inférieur à 60 caractères.</p>}
            <button
